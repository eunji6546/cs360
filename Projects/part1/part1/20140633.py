...
def parseJson(json_file):
    with open(json_file, 'r') as f:
        dUser = open('Users.dat','a')
        # uID, Rating, Loc, Count
        dCate = open('Categories.dat','a')
        # Category
        dPrice = open('Prices.dat','a')
        # ItmeId, Firstbid, Buyprice, Currently
        dItem = open('Items.dat', 'a')
        # Itemid, seller Id, name, dscr, num of bids
        dBids = open('Bids.dat','a')
        # Itemid, buyerID, Amount, Time
        dTimes = open('Times.dat','a')
        # Itemid, started, end
        ...
        for item in items['Item']:
            ...
            #create table Item( ItemID int primary key,  SellerID varchar, Name varchar, Description varchar, Num_of_Bids int);
            dItem.write(item_id + columnSeparator + makeitString(seller_id)+ columnSeparator
                        + makeitString(name)+columnSeparator + makeitString(description) +columnSeparator+num_of_bid+"\n")
            ...
            if num_of_bid != "0":
                for bid_info in item['Bids']['Bid']:
                    ...
                    dBids.write(item_id+"|"+makeitString(bid_info['Bidder']['_UserID'])+"|"+transformDttm(bid_info['Time'])+"|"+transformDollar(bid_info['Amount'])+"\n")
                    ...
                    dUser.write(makeitString(buyer_info['_UserID'])+"|"+buyer_info['_Rating']+"|"+bidder_location+"|"+bidder_country+"\n")
            ...
            dPrice.write(item_id+"|"+first_bid+"|"+buy_price+"|"+currently+"\n")
